		...
		success : function(data) {
			...
			for (var j = 0; j < days.length; j++) {
				for (var i = 0; i < data.list.length; i++) {
					var time = Format(data.list[i].create_time, "yyyy-MM-dd");
					if (days[j] == time) {
...
var initBlogCountByDate = function(startTime, endTime) {
	...
	$.ajax({
		url : '../blogadmin/getbloglistbydate',
		type : 'post',
		data : params,
		dataType : 'json',
		success : function(data) {
...
var initBlogClickSort = function() {
	$.ajax({
		...
		success : function(data) {
			...
			idArray[1] = new Array();
			for (var i = 0; i < data.list.length; i++) {
				idArray[0][i] = data.list[i].blog_id;
				counts[i] = data.list[i].click_num;
			}
			for (var i = 0; i < data.list.length; i++) {
				idArray[1][i] = data.list[i].click_num;
			}
...
function selectBlogById(blogId) {
	...
	$.ajax({
		...
		success : function(data) {
			//查询成功
			if (data.success) {
				$(".newsview").find(".news_title").html(data.blog.blogTitle);
				$(".newsview").find(".au02").html(Format(data.blog.createTime, "yyyy-MM-dd hh:mm:ss"));
				$(".au03").find('b').html(data.blog.clickNum);
				$(".news_about").find(".news_intr").html(data.blog.blogIntroduction);
				var keyword = '';
				...
				$(".newsview").find(".news_infos").html(data.blog.blogContent);
			}
